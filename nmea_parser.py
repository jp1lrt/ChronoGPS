...
class NMEAParser:
    ...
    def parse(self, nmea_sentence):
        if not nmea_sentence.startswith('$'): return None
        parts = nmea_sentence.split(',')
        msg_type = parts[0]
        
        if 'RMC' in msg_type: return self._parse_rmc(parts)
        elif 'GGA' in msg_type: self._parse_gga(parts)
        elif 'GSA' in msg_type: self._parse_gsa(parts)
        elif 'GSV' in msg_type: self._parse_gsv(parts, msg_type)
        return None

    def _parse_rmc(self, parts):
        try:
            if len(parts) < 10 or parts[2] != 'A': return None
            dt = datetime.strptime(parts[9] + parts[1][:6], "%d%m%y%H%M%S").replace(tzinfo=timezone.utc)
            if self.last_time_update == dt: return None
    ...
    def _parse_gga(self, parts):
        try:
            if len(parts) > 9 and parts[2] and parts[4]:
                ...
                self._calculate_grid_locator()
            if len(parts) > 9 and parts[9]: self.altitude = float(parts[9])
        except: pass
    ...
    def _parse_gsa(self, parts):
        ...
        try:
            msg_header = parts[0]
            if '$GP' in msg_header: sys_id = 1
            elif '$GL' in msg_header: sys_id = 2
            elif '$GA' in msg_header: sys_id = 3
            elif '$GB' in msg_header: sys_id = 4
            elif '$GQ' in msg_header: sys_id = 6
            else:
    ...
    def _parse_gsv(self, parts, msg_type):
        ...
        try:
            ...
            # Talker IDによる絶対判定
            if '$GP' in msg_type: sys_id = 1    # GPS
            elif '$GL' in msg_type: sys_id = 2 # GLONASS
            elif '$GA' in msg_type: sys_id = 3 # Galileo
            elif '$GB' in msg_type: sys_id = 4 # BeiDou
            elif '$GQ' in msg_type: sys_id = 6 # QZSS
            else: sys_id = 1 # 不明はGPSへ
    ...
    def get_satellites_by_system(self):
        ...
        for s in res: res[s].sort(key=lambda x: int(x['id']))
        return res
    ...
    def _calculate_grid_locator(self):
        ...
        try:
            ...
            grid += chr(int((lon%(2/240))/(2/5760))+ord('a')) + chr(int((lat%(1/240))/(1/5760))+ord('a'))
