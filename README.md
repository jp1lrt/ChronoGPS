...
管理者権限で自動起動させたい場合は [FAQ Q21](docs/FAQ.md#q21) を参照してください。
...
1. PCにGNSSレシーバーを接続する（USB GPSレシーバー推奨）
2. `ChronoGPS.exe` を起動 — デフォルトは**モニタ専用モード**で起動します
3. 画面上部のバナー **「時刻同期を有効にする（管理者として再起動）」** をクリック
4. COMポートとボーレート（デフォルト: 9600）を選択して **開始** をクリック
5. **GPS同期** をクリックするか **即時同期** を有効にする — GNSSを継続参照して同期開始

> COMポートがリストに表示されない場合は、直接入力できます（例：`COM16`）。  
> NTPのみ使用する場合は手順1・4をスキップし、NTPサーバーを入力して **NTP同期** をクリックしてください。
...
ChronoGPS は GNSS（GPS / QZSS など）を **UTC に直結した絶対時刻基準**として使用します。

FT8 / FT4 などの **通常のデジタルモード運用**では、  
**GPS の「Instant Sync（即時同期）」を推奨**します。
...
> （＝不要な連続補正をしない）という意味です。
>
> **これは「同期をオフにしてよい」という意味ではありません。**
> Instant Sync は **有効にしたまま運用して問題ありません**。
>
> Instant Sync は GNSS を常に参照しますが、  
> **必要な場合にのみ最小限の補正**が行われます。
>
> また、**GPS受信を停止してよいという意味でもありません。**
> GPS受信は表示・監視・検証目的で継続していても問題ありません。
...
![Weak Sync（定期同期）動作図](docs/weak-sync-diagram.ja2.png)
...
>
> 閾値内であれば補正を行わず、  
> **GNSS受信ジッタを Windows のシステム時刻に注入しない**
> ことを優先します。
...
第三者同梱（バンドル）・再配布ポリシー
- 他のアプリ／インストーラに ChronoGPS を同梱する場合は、可能な限り **GitHub Releases で公開している公式配布物**（`checksums.txt` で検証できるもの）をそのまま使用してください。
- やむを得ず再ビルドする場合は、**ビルド識別子**（例：ビルド環境やコミット等）と **配布物のSHA256ハッシュ**を公開してください。（同じバージョン番号で別バイナリが出回ると、検証・サポートが非常に困難になります）
- 公式リリースと誤認される形での改変配布は避けてください。

サポート範囲（問い合わせ先）
- **公式Release** で再現する不具合は、本リポジトリの Issue で歓迎します。
- 第三者の同梱・起動・インストール・権限・設定保存先・更新挙動など、**同梱側の実装／配布手順に起因する問題**は、まず同梱元プロジェクト側で切り分け・対応してください。
...
- VirusTotal スキャン結果 (v2.5.2): https://www.virustotal.com/gui/file/37e30dfe94bdc07c6e75dcb8fbf055012a908edf6a0f6fe0ed473f50485b3ef4/detection  
  （72エンジン中4件検出 — Arctic Wolf / Bkav Pro / DeepInstinct / SecureAge によるヒューリスティック誤検知）
