...
**GPSの「Instant Sync（即時同期）」を行えば通常は十分です。**

Instant Sync は、

- GNSS を基準として  
- **OS のシステム時刻を適切な代表値で一度校正する**

ための同期モードです。

ここで重要なのは、Instant Sync が  
**「毎秒追従」や「連続補正」を行うモードではない** という点です。

> [!NOTE]
> **Instant Sync が「十分」であるという意味**
>
> 「Instant Sync を使えば十分」という表現は、  
> **OS のシステム時刻を書き換える操作を、頻繁に繰り返す必要がない**
> という意味です。
>
> **これは「同期をオフにしてよい」という意味ではありません。**
> **必要なときに、適切な代表値で一度だけ校正すればよい**という意味です。
>
> **GPS受信を停止してよい、という意味ではありません。**
> GPS受信自体は、表示・監視・検証目的で継続していても問題ありません。

GNSS は非常に安定した時刻基準であり、
運用開始時に正しく校正しておけば、
その後の FT8 / FT4 運用で問題となるレベルの時刻ずれが生じることは通常ありません。
